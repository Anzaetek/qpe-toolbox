...
def update_params_from_partial(circ, tn):
    """
    Update circuit parameters from a partially optimized tensor network.

    Parameters
    ----------
    circ : quimb.tensor.Circuit or quimb.tensor.CircuitMPS
        Circuit whose gate parameters will be updated.
    tn : quimb.tensor.TensorNetwork
        Tensor network containing updated parametrized gate tensors.

    Notes
    -----
    - Only parametrized gates are updated.
    - The update is performed **in place**.
    - Cached circuit data is cleared after updating.

    """
    for i, gate in enumerate(circ._gates):
        tag = circ.gate_tag(i)
        ...
        if tag in tn.tags:
            ...
            if isinstance(t, qtn.PTensor):
                ...
                circ._psi[tag].params = t.params

                # update the circuit's gate record
                circ._gates[i] = qtn.Gate(
                    label=gate.label,
                    params=t.params,
                    qubits=gate.qubits,
                    round=gate.round,
